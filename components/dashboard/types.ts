...
export const getDepartamentoByResponsavel = (responsavel: string): Departamento | null => {
  return DEPARTAMENTOS.find(dep => dep.responsavel.toLowerCase() === responsavel.toLowerCase()) || null;
};

export const getResponsavelInfo = (nome: string): ResponsavelInfo | null => {
  return RESPONSAVEIS.find(resp => resp.nome.toLowerCase() === nome.toLowerCase()) || null;
};
