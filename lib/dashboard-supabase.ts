...
export async function saveDashboardData(
  ...
  try {
    ...
    const { error: dashboardError } = await supabase.from("dashboard_data").insert({
    ...
    await supabase.from("dashboard_uploads").update({ status: "completed" }).eq("id", uploadRecord.id)
    ...
    if (error?.code === "42P01" || error?.code === "PGRST116") {
      ...
      alert("Erro ao salvar dados do dashboard. Verifique o console para mais detalhes.")
    }
...
export async function loadDashboardData(): Promise<{
  ...
  try {
    ...
    if (dashboardError && dashboardError.code !== "PGRST116" && dashboardError.code !== "42P01") {
...
export async function getDashboardItemsByCategory(category: string): Promise<DashboardItem[]> {
  try {
    ...
    switch (category) {
      case "aprovacao":
        ...
        break
      case "analises":
        query = query.or("status.ilike.%análise%,status.ilike.%analise%,status.ilike.%revisão%,status.ilike.%revisao%")
        break
      case "orcamentos":
        query = query.or(
